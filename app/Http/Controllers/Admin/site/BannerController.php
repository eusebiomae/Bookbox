...
class BannerController extends BaseMethodSystemController
{
	...
	{
		$list = LeadsModel::withTrashed()->with('course')->where('flg_type', $request->flg_type);
		...
		if (empty($request->get('course')) && empty($request->get('is_formed_in_psychology'))) {
			$list->whereRaw('created_at > CURRENT_DATE - INTERVAL 3 MONTH');
		}
